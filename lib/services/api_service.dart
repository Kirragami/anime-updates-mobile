...
class ApiService {
  ...
  /// Fetch a paginated page of tracked shows (requires auth)
  Future<Map<String, dynamic>> fetchTrackedShowsPage({required int page, required int size}) async {
    try {
      final uri = Uri.parse('${AppConstants.baseUrl}${AppConstants.trackedShowsListEndpoint}').replace(
        queryParameters: {
          'page': page.toString(),
          'size': size.toString(),
        },
      ...
      if (kDebugMode) {
        print('Making paginated tracked shows API call to: $uri');
      }
      ...
      if (kDebugMode) {
        print('Tracked shows API call error: $e');
      }
      rethrow;
    }
  }
  ...
  Future<List<AnimeItem>> searchAnime(String searchText) async {
    try {
      final endpoint = '${AppConstants.baseUrl}/api/anime/$searchText/get-releases';
      
      if (kDebugMode) {
        print('Making search API call to: $endpoint');
      }
      ...
      if (response.statusCode == 200) {
        ...
        if (responseBody.isEmpty) {
          if (kDebugMode) {
            print('Empty search response received');
          }
          return [];
        }
        ...
        if (kDebugMode) {
          print('Successfully created ${animeList.length} anime items from search');
          if (animeList.isNotEmpty) {
            print('First item: ${animeList.first.title}');
          }
        }
