...
class AnimeGridCard extends ConsumerWidget {
  ...
  Widget _buildTrackingIndicator(WidgetRef ref) {
    // Use the tracked field directly from the anime item
    if (!anime.tracked) {
  ...
  Widget _buildActionButtons(WidgetRef ref) {
    // Use the new download status provider instead of old DownloadManager
    final downloadStatus = ref.watch(downloadStatusProvider(anime.id));
